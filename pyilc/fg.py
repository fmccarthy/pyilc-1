...
def get_mix(nu_ghz, comp, param_dict_file=None, param_dict_override=None,
    ...
    if (comp == 'CMB' or comp == 'kSZ'): #CMB (or kSZ)
        ...
        resp[np.where(nu_ghz == None)] = 0. #this case is appropriate for HI or other maps that contain no CMB-relevant signals (and also no CIB); they're assumed to be denoted by None in nu_ghz
        return resp
    elif (comp == 'patchydark_tau_incorrect'): #patch dark screening (from https://arxiv.org/pdf/2307.15124.pdf) #Fiona remove correct at end
        nu = 1.e9*np.asarray(nu_ghz).astype(float)
        X = hplanck*nu/(kboltz*TCMB)
        nu0 = 353e9
        X0 = hplanck*nu0/(kboltz*TCMB)
...
def get_test_fdict():
    ...
    comps = ['CMB','kSZ','tSZ','mu','rSZ','CIB','CIB_Jysr','radio','radio2','radio_Jysr','radio2_Jysr','CIB_Jysr_dbeta','CIB_dbeta','CIB_dT','CIB_Jysr_dT','patchydark_tau_correct'] 
